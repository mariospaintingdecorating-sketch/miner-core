...
try {
  check('packaged application version and identity', () => {
    ...
    assert.equal(pkg.version, '0.2.3-beta');
  ...
  fs.writeFileSync(path.join(out, 'Core-Miner-0.2.3-production-audit.json'), audit.stdout || JSON.stringify({ error: audit.error?.message || audit.stderr }));
  ...
  fs.writeFileSync(path.join(out, 'Core-Miner-0.2.3-package-verification.json'), JSON.stringify(report, null, 2));
